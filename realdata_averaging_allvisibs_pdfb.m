...
visibility_file_name = 'data/vis/AVERAGED-CYGA-C-6680-64CH/10snapshots/';
...
nufft_param.use_fft_on_gpu = 0; % gpu FFT
...
%% get input data
script_get_input_averaged_data;
B = A; Bt = At;
...
param_pdfb.rel_obj = 0; %1e-5; % stopping criterion
...
param_pdfb.adapt_bound_rel_obj = 1e-4;
param_pdfb.hard_thres = 0;
param_pdfb.adapt_bound_tol =1e-3;
param_pdfb.adapt_bound_start = 500;
...
filename = sprintf('results/averaging/i%d.dl%d.n%05d.eps%d.gamma%d.hyperspec.q6-5-51.averaging.', param_real_data.image_size_Ny, (10*param_real_data.pixel_size), param_pdfb.max_iter, step_epsilon, abs(log10(param_pdfb.gamma)));
...
peakpsfA = max(max((real(At(Gw' * Gw * A(delta)))))); % Peak of PSF = Peak(Phi^T(Phi(delta)))
