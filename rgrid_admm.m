function [SNR, DR, M, histogrampeakiness, tend] = rgrid_admm(visibSize, input_snr, imgfile, coveragefile, run)
% inputs
...
solfile=sprintf('results/%s/soln.rgrid.i%d.p%02d.r%02d.mat', subdirname, Ny, ceil(visibSize/(N)), run);
logfile=sprintf('logs/%s/soln.rgrid.i%d.p%02d.r%02d.log', subdirname, Ny, ceil(visibSize/(N)), run);
diary(logfile);
...
% This is to avoid having VERY small values close to numerical precision 
% which might later explode during computation of inverse or reciprocal.
